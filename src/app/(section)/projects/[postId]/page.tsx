...
export default async function Post({params: {postId}}: Props) {
    ...
    const tags = meta.tags.map((tag, i) => (
        <React.Fragment key={i}>
            <Link className='text-MyRed md:text-lg text-sm' href={`/tags/${tag}`}
    ...
    return (
        <main
            className='mt-24 md:mt-40 px-4 md:px-6 prose  prose-invert prose-xl prose-slate dark:prose-invert mx-auto max-w-[1600px]'>
